...
- **Component** `analysis/Analysis.jsx` composing **5** sub-components: `BreakdownTable`, `QuizHistory`, `FinalExams`, `LastQuizSummary`, `QuestionAttemptsTable`
- **CSS** `Analysis.css` (page layout: head, scorecard, sections, desktop rail, page furniture) + `analysisPanels.css` (every panel, `.ap-*`) +shell. Nothing else.
- **2026-09 report rebuild (current layout).** One report instead of six accordions: a scorecard hero (overall accuracy above the bank-coverage meter it has to be read against, exam countdown and streak beside it, then the "do this next" actions), then five always-open sections — by specialty, by source, activity (figures + quiz history), mock exams, last quiz review — with a sticky section rail on desktop only (≥1024px). Specialty and source rows are one `BreakdownTable` (`.ap-tbl*`) carrying **accuracy (green/amber/red ramp) and coverage (plain blue bar) on one row**, sortable weakest / least covered / name; untouched rows show "not started", never a red 0%. Quiz history and mock exams share one session-list shape (`.ap-sess*`) and one review card (`.ap-review`). Source rows come from the progress endpoint's live bank, so retired collections (October25 etc.) no longer show a permanent 0%.
- **Deleted in the rebuild** `Progress.jsx/.css`, `OverallStats.jsx`, `TopicAnalysisTable.jsx`, `QuizHistory.css`, `FinalExams.css` (~2,000 lines of glassmorphism-era CSS). Also fixed on the way: quiz-history details called a non-existent `/quiz-sessions/:id/details` route (it is `/quiz-sessions/:id`), and the old "readiness" figure read `total_correct_options`, a field the API never returned.
- **History below this line** describes the intermediate 2026-09 drill layout, kept for the reasoning.
