/**
 * Bulk insert November 2025 questions into the database.
 * Reads from all JSON files in ../../source-material/JSON/nov25/
 * 
 * Usage:
 *   With DATABASE_URL:
 *     DATABASE_URL=postgres://user:pass@host:port/db node scripts/insertNovemberQuestions.js
 *   With individual vars (same as backend):
 *     DBUSER=x DBHOST=x DBNAME=x DBPASSWORD=x DBPORT=x node scripts/insertNovemberQuestions.js
 */
...
const SOURCE_KEY = 'November25';
const BATCH_SIZE = 100;

async function main() {
    const dirPath = path.join(__dirname, '..', '..', 'source-material', 'JSON', 'nov25');

    if (!fs.existsSync(dirPath)) {
        console.error(`ERROR: November directory not found at ${dirPath}`);
        process.exit(1);
    }

    // Read all JSON files from November directory
    const files = fs.readdirSync(dirPath).filter(f => f.endsWith('.json'));
    console.log(`Found ${files.length} JSON files in November directory.`);

    if (files.length === 0) {
        console.error('ERROR: No JSON files found in November directory.');
        process.exit(1);
    }
    ...
    try {
        // Update the source constraint to allow November25
        ...
        // Add new constraints with November25 included
        await client.query(`
            ALTER TABLE user_quiz_sessions 
        ...
        // Check how many November25 questions already exist
