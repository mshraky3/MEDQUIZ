...
async function main() {
    ...
    await section('Cross-check — must match the live admin panel', async () => {
        ...
        log(table(['metric', 'value'], [
            ...
            ['Accounts that tried', conv.totalTried],
            ['Tried → paid', conv.triedToPaid],
            ['Conversion rate', `${conv.conversionRate}%`],
            ['Active subscribers', n(sub.active_subscribers)],
            ['Free tier, questions left', n(sub.free_trying)],
            ['Free tier, allowance used up', n(sub.free_exhausted_unconverted)],
