...
// ─── Unsubscribe token ─────────────────────────────────────────────────────
// Its own secret, not ADMIN_KEY: this HMAC key and the admin panel's access
// key protect different things, and reusing one for both means a leak of
// either lets an attacker forge the other (unsubscribe tokens from the admin
// key, or vice versa). If UNSUBSCRIBE_SECRET isn't set, generate one
// per-process instead of falling back to a literal sitting in this file
// (which would let anyone with repo access compute a valid unsubscribe token
// for any account id) — no env var required, and nothing guessable from the
// source. The one real cost: a link generated by one server instance stops
// working if a later request lands on a different instance's freshly-
// generated secret. Worth it here — forging this token only unsubscribes an
// account from marketing mail without consent, not an account-security
// issue, so the fix that needs zero deploy steps wins over a permanently
// stable secret that needs one.
const _generatedUnsubSecret = crypto.randomBytes(32).toString('hex');
function unsubSecret() {
    return process.env.UNSUBSCRIBE_SECRET || _generatedUnsubSecret;
}
